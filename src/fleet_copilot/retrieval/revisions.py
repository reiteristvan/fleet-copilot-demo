...
The corpus carries a planted conflict: two maintenance procedures giving
different maximum battery charging temperatures for the SD-50B, revisions 3 and
4, both live. Retrieval that returns revision 3 hands a technician a withdrawn
...
nothing that groups documents into a series -- no family id, no `supersedes`
link. Revision 3 of *what* is the question, and it has to be inferred.
...
and it is badly wrong on this corpus: it forms 29 groups and marks 68 of 120
documents superseded. One of those groups holds eight handover notes for the
same machine and part -- separate shift events written on different days at
different sites, every one of them revision 1, none a revision of any other.
Demoting seven of them would bury correct answers behind a ranking that looks
entirely reasonable.

The rule here trusts a naming convention, and its failure mode is missing a
supersession that was never named -- leaving a stale document at normal rank
rather than hiding a live one. That is the direction worth failing in (ADR 0009).
"""
...
def family_stem(doc_id: str) -> str:
    ...
    suffix is its own stem, and therefore its own family.
    """
...
def current_doc_ids(entries: Iterable[ManifestEntry]) -> set[str]:
    ...
    tie is not evidence that either withdrew the other, and breaking it would
    hide a real document to satisfy the rule.
    """
